"""
Trial 5: Balanced Real Data Sampling for EMG gesture recognition

Strategy:
- Remove Pinch class (1 sample only)
- Sample each class equally (real data only, NO SMOTE)
- Target: smallest class size (Open: 606 samples) × 5 classes = ~3,030 samples
- Perfect balance: 20% per class
- Avoids Trial 1/4 SMOTE overfitting
- Avoids Trial 3 extreme data loss
"""
import sys
...
def balanced_sampling(X, y, remove_pinch=True, random_state=42):
    """
    Create perfectly balanced dataset by sampling each class equally

    Trial 5 Strategy:
    - Remove Pinch class (1 sample)
    - Find minimum class size (Open: 606 samples after removing Pinch)
    - Sample all other classes to match minimum size
    - Result: Perfect 20% balance per class, ~3,030 total samples
    - NO SMOTE - real data only

    Args:
        X: Features (n_samples, n_channels, seq_length)
        y: Labels (n_samples,)
        remove_pinch: Remove Pinch class (default: True)
    ...
    print("\n" + "="*80)
    print("TRIAL 5: BALANCED REAL DATA SAMPLING")
    print("="*80)
    ...
    for label, count in zip(unique_labels, counts):
        print(f"  Class {label}: {count:5d} samples ({count/len(y)*100:5.2f}%)")

    # Remove Pinch class if requested
    if remove_pinch:
        pinch_label = 5
        mask = y != pinch_label
        X = X[mask]
        y = y[mask]
        print(f"\n[Pinch Removal]")
        print(f"  Removed Pinch class (label {pinch_label})")
    ...
    print(f"  Minimum class size: {min_samples} samples")
    print(f"  Target per class: {min_samples} samples")
    print(f"  Number of classes: {len(unique_labels)}")
    print(f"  Total target samples: {min_samples * len(unique_labels)}")
    ...
    for label in unique_labels:
        ...
        if len(y_class) > min_samples:
            indices = np.random.choice(len(y_class), min_samples, replace=False)
            X_class_sampled = X_class[indices]
            y_class_sampled = y_class[indices]
        else:
            X_class_sampled = X_class
            y_class_sampled = y_class

        X_balanced_list.append(X_class_sampled)
        y_balanced_list.append(y_class_sampled)
        print(f"  Class {label}: {len(y_class)} -> {len(y_class_sampled)} samples")
    ...
    for label, count in zip(unique_labels, counts):
        print(f"  Class {label}: {count:5d} samples ({count/len(y_balanced)*100:5.2f}%)")

    print(f"\n[Dataset Size Change]")
    print(f"  Original: {len(y)} samples (after Pinch removal)")
    print(f"  Balanced: {len(y_balanced)} samples")
    print(f"  Reduction: {len(y) - len(y_balanced)} samples ({(1 - len(y_balanced)/len(y))*100:.1f}%)")
    print("="*80 + "\n")
...
class Trainer:
    ...
    def __init__(self, model, device, train_loader, val_loader, test_loader,
        ...
        self.class_names = ['No Gesture', 'Fist', 'Wave In',
                           'Wave Out', 'Open']  # Removed Pinch class
...
def main(args):
    ...
    # ===============================
    # 2.5. Apply balanced sampling
    # ===============================
    print("\n[Step 2.5/5] Applying balanced sampling...")
    X_train_balanced, y_train_balanced = balanced_sampling(
        X_train_preprocessed, y_train, remove_pinch=True, random_state=args.random_state
    )
    X_test_balanced, y_test_balanced = balanced_sampling(
        X_test_preprocessed, y_test, remove_pinch=True, random_state=args.random_state
    )

    # Train/Val split
    X_train, X_val, y_train_split, y_val = create_data_split(
        X_train_balanced, y_train_balanced, test_size=args.val_split, random_state=args.random_state
    )

    print(f"\nTrain: {X_train.shape}, Val: {X_val.shape}, Test: {X_test_balanced.shape}")
    ...
    train_loader, val_loader, test_loader = create_dataloaders(
        X_train, y_train_split, X_val, y_val, X_test_balanced, y_test_balanced,
    ...
    model = get_model(
        ...
        input_channels=8,
        num_classes=5,  # 5 classes after removing Pinch
    ...
    trainer = Trainer(
        ...
        save_dir=f'results/trial5_balanced_{args.model_type}'
    )
