...
アーキテクチャ探索（4モデル）の結果、問題の根源がデータ不均衡にあることが明確になったため、データレベルの対策を系統的に試行。

---

### 📊 Trial 4: 穏やかな組み合わせリサンプリング (combined-gentle-resampling branch)

**戦略**:
- Trial 3の過度なアンダーサンプリング（82%削減）を改善
- **Step 1**: No Gestureを穏やかにアンダーサンプリング: 11,117 → 6,100 (45%削減)
- **Step 2**: 少数派クラス（Fist、Wave In、Open）をSMOTEで1,000サンプルに増強
- **データ保持率**: 77.6% (vs Trial 3の42%)

**訓練設定**:
```bash
python train_combined_resampling.py --model_type cnn_lstm --epochs 15
```

### 訓練結果
- **検証精度（ベスト）**: 13.15% (Epoch 7で達成)
- **検証精度（最終）**: 9.03% (Epoch 15)
- **訓練精度（最終）**: 49.21%

### 🚨 **Trial 4: 完全失敗 - SMOTEの致命的欠陥を再確認**

#### 失敗の根本原因

1. **SMOTEによる過学習**:
   - 訓練精度: 49%（モデルが訓練データすら学習できず）
   - 検証精度: 9-13%（Trial 1の17.32%よりさらに悪化）
   - SMOTEの合成データが汎化能力を完全に破壊

2. **穏やかなアプローチも無意味**:
   - Trial 3: 82%削減 → 46.51%精度
   - Trial 4: 45%削減 + SMOTE → 13.15%精度（**最悪結果**）
   - データ保持率を上げても、SMOTEが全てを台無しに

3. **検証損失の爆発**:
   - Epoch 1: 1.79
   - Epoch 15: 3.01（1.68倍に悪化）
   - モデルが学習とは逆方向に退化

#### 決定的教訓

**SMOTEは完全に使用禁止**:
- Trial 1（純SMOTE）: 17.32%
- Trial 4（穏やか + SMOTE）: 13.15%
- どんな組み合わせでもSMOTEは失敗を保証

**データ戦略の再考が必須**:
- リアルデータのみの完全バランシングを試す必要性
- 合成データ生成の完全排除

---

### 📊 Trial 5: 完全バランスリアルデータサンプリング (balanced-real-data branch)

**戦略**:
- Pinchクラスを除外（1サンプルのみ）
- 最小クラスサイズ（Wave In: 221サンプル）に全クラスを合わせる
- **完全バランス**: 各クラス20%（No Gesture, Fist, Wave In, Wave Out, Open）
- **SMOTEを完全排除** - リアルデータのみ使用

**データセット変化**:
```
訓練セット:
  元データ: 7,835サンプル (Pinch除外後)
  バランス後: 1,105サンプル (85.9%削減)
  各クラス: 221サンプル (完全均等)

テストセット:
  元データ: 7,773サンプル
  バランス後: 745サンプル (90.4%削減)
  各クラス: 149サンプル (完全均等)
```

**訓練設定**:
```bash
python train_balanced.py --model_type cnn_lstm --epochs 15
```

### 訓練結果
- **検証精度（ベスト）**: 22.62% (Epoch 15で達成)
- **テスト精度**: 25.37%
- **訓練精度（最終）**: 20.59%
...
Classification Report (Test Set):
  ...
  No Gesture       0.39      0.31      0.34       149
        Fist       0.33      0.06      0.10       149
     Wave In       0.00      0.00      0.00       149
    Wave Out       0.00      0.00      0.00       149
        Open       0.22      0.90      0.36       149

    accuracy                           0.25       745
   macro avg       0.19      0.25      0.16       745
weighted avg       0.19      0.25      0.16       745
```

### 🚨 **Trial 5: 失敗だが異なる失敗モード**

#### 観察された現象

1. **予測パターンの変化**:
   - **ベースライン**: 常に "No Gesture" を予測（Recall 1.00）
   - **Trial 5**: 主に "Open" を予測（Recall 0.90）
   - 多数派予測から脱却したが、別のクラスへの偏向が発生

2. **完全に無視されるクラス**:
   - Wave In: Recall 0.00（149サンプルすべて誤分類）
   - Wave Out: Recall 0.00（149サンプルすべて誤分類）
   - 完全バランスでも一部クラスを学習できず

3. **データサイズの問題**:
   - 訓練: 884サンプル（バランス後の80%）
   - 検証: 221サンプル
   - CNN-LSTMが学習するには不十分なデータ量

#### 失敗の根本原因

1. **過度なデータ削減**:
   - 85.9%のデータを捨てた結果、情報量が不足
   - 各クラス221サンプルでは特徴パターンを学習できない
   - CNN-LSTM (791K params) には小さすぎるデータセット

2. **完全バランスの副作用**:
   - 現実のデータ分布（No Gesture 70%, Fist 3%）を無視
   - モデルが現実的な事前分布を学習できない
   - テストセット（実データ分布）とのミスマッチ

3. **クラス間の学習難易度差**:
   - Open: 学習しやすい（Recall 0.90）
   - Wave In/Out: 学習困難（Recall 0.00）
   - 均等サンプリングでは難しいクラスの学習が不十分

#### Trial 5の価値

**失敗から得られた知見**:
- SMOTEを排除した効果: 22.62% (vs Trial 4の13.15%)
- しかし完全バランスは逆効果（vs ベースライン70.15%）
- リアルデータのみが正しい方向だが、戦略を再考が必要

**次のステップへの示唆**:
- データ削減率を下げる必要性（85.9%は過剰）
- 部分的なバランシング（例: No Gesture 70% → 50%）
- より大きなデータセットの確保
