"""
Deep Learning training with SMOTE resampling for class imbalance
Trial 1: Data-level intervention
"""
import sys
...
def apply_smote_resampling(X, y, strategy='auto', random_state=42):
    """
    ...
    """
    # Reshape X for SMOTE: (n_samples, n_channels * seq_length)
    ...
    # Apply SMOTE + Tomek links (combined oversampling + undersampling)
    print(f"\nApplying SMOTE + Tomek resampling...")
    smote_tomek = SMOTETomek(random_state=random_state)
    X_resampled, y_resampled = smote_tomek.fit_resample(X_flat, y)

    # Reshape back to original format
    X_resampled = X_resampled.reshape(-1, n_channels, seq_length)
    ...
    print(f"\nDataset size: {len(y)} → {len(y_resampled)} ({len(y_resampled)/len(y)*100:.1f}%)")
...
def remove_pinch_class(X, y, pinch_label=5):
    ...
    Args:
        X: Features
        y: Labels
        pinch_label: Label to remove (default: 5)
    ...
    print(f"\nRemoving Pinch class (label {pinch_label}):")
    print(f"  Original: {len(y)} samples")
    print(f"  Filtered: {len(y_filtered)} samples")
...
class Trainer:
    ...
    def __init__(self, model, device, train_loader, val_loader, test_loader,
                 ...
                 save_dir='results/smote_resampling'):
        self.model = model
        self.device = device
    ...
    def train(self):
        ...
        print(f"\n{'='*80}")
        print("Starting Training with SMOTE Resampling")
        print(f"{'='*80}")
...
def main(args):
    ...
    print("="*80)
    print("EMG Gesture Recognition - SMOTE Resampling (Trial 1)")
    print("="*80)
    ...
    # ===============================
    # 3. Remove Pinch class
    # ===============================
    print("\n[Step 3/6] Removing Pinch class...")
    X_train_no_pinch, y_train_no_pinch = remove_pinch_class(X_train_preprocessed, y_train)
    X_test_no_pinch, y_test_no_pinch = remove_pinch_class(X_test_preprocessed, y_test)

    # Train/Val split
    X_train, X_val, y_train_split, y_val = create_data_split(
        X_train_no_pinch, y_train_no_pinch, test_size=args.val_split, random_state=args.random_state
    )

    print(f"\nTrain: {X_train.shape}, Val: {X_val.shape}, Test: {X_test_no_pinch.shape}")

    # ===============================
    # 4. Apply SMOTE resampling
    # ===============================
    print("\n[Step 4/6] Applying SMOTE resampling...")
    X_train_resampled, y_train_resampled = apply_smote_resampling(
        X_train, y_train_split, random_state=args.random_state
    )

    # ===============================
    # 5. Create DataLoaders
    # ===============================
    print("\n[Step 5/6] Creating DataLoaders...")
    train_loader, val_loader, test_loader = create_dataloaders(
        X_train_resampled, y_train_resampled, X_val, y_val, X_test_no_pinch, y_test_no_pinch,
    ...
    # ===============================
    print("\n[Step 6/6] Creating model and training...")
    model = get_model(
        model_type=args.model_type,
        input_channels=8,
        num_classes=5,  # 5 classes (removed Pinch)
        dropout=args.dropout
    ).to(device)

    # No class weights needed with SMOTE (balanced dataset)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train with SMOTE resampling')
